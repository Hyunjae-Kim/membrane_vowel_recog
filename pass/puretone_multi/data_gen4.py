...
for c in ch_list:
    ...
    for freq in range(650,750,5):
        ...
        for try_num in range(1000):
            sound = np.sin(2*np.pi*(freq*np.linspace(0,time_len/1000,itmax)+np.random.random()))*amp  ##w
            sound = sound + 0.1*np.random.random(len(sound))*0                                        ## _clean
        ...
        if ch_num==9:
            np.save('data/pure_data%d/w_raw_clean/%d.npy'%(data_num, freq), whole_raw)
