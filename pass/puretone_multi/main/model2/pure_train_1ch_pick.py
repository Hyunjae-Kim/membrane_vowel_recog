...
parser.add_argument('--pick_ch', type=int)
...
pick_ch = args.pick_ch
ch_num = 9   #for loading data
...
if __name__=='__main__':
    print('[Training]%dch_%.1f_model%d'%(pick_ch, time_len, model_num))
    torch.manual_seed(37)
    torch.cuda.manual_seed_all(37)
    torch.backends.cudnn.deterministic = True
    
    trainX = np.load('../../npy_data/pure_data%d/%dch/%dch_%.1fms_trainX.npy'\
                     %(data_num, ch_num, ch_num, time_len))[:, pick_ch-1].reshape(-1,1,point_len)
    trainY = np.load('../../npy_data/pure_data%d/%dch/%dch_%.1fms_trainY.npy'\
                     %(data_num, ch_num, ch_num, time_len))
    testX = np.load('../../npy_data/pure_data%d/%dch/%dch_%.1fms_testX.npy'\
                    %(data_num, ch_num, ch_num, time_len))[:, pick_ch-1].reshape(-1,1,point_len)
    testY = np.load('../../npy_data/pure_data%d/%dch/%dch_%.1fms_testY.npy'\
                    %(data_num, ch_num, ch_num, time_len))
    ...
    print('%dch data shape  -  %.1f ms'%(pick_ch, time_len))
...
#     model.load_state_dict(torch.load('ckpt/model%d_mem/%.1f_ckpt_2000.pt'%(model_num, time_len)))
    ...
    for epoch in range(1001):
        ...
        if epoch%100==0:
            ...
            torch.save(model.state_dict(),'../../ckpt/model%d/pure_data%d/1ch_pick/ch%d/%.1f_ckpt_%d.pt'\
                       %(model_num, data_num, pick_ch, time_len, epoch))
            np.savetxt('../../result/model%d/pure_data%d/1ch_pick/ch%d/%.1f_loss_tr.txt'\
                       %(model_num, data_num, pick_ch, time_len), train_loss)
            np.savetxt('../../result/model%d/pure_data%d/1ch_pick/ch%d/%.1f_loss_te.txt'\
                       %(model_num, data_num, pick_ch, time_len), test_loss)
            np.savetxt('../../result/model%d/pure_data%d/1ch_pick/ch%d/%.1f_acc_tr.txt'\
                       %(model_num, data_num, pick_ch, time_len), train_acc)
            np.savetxt('../../result/model%d/pure_data%d/1ch_pick/ch%d/%.1f_acc_te.txt'\
                       %(model_num, data_num, pick_ch, time_len), test_acc)
