...
if __name__=='__main__':
    ...
    trainX = np.load('../../../npy_data/set_data2/%dband/%ddb_mic_trainX.npy'\
                     %(n_band, SNR))
    trainY = np.load('../../../npy_data/set_data2/%dband/%ddb_mic_trainY.npy'\
                     %(n_band, SNR))
    testX = np.load('../../../npy_data/set_data2/%dband/%ddb_mic_testX.npy'\
                    %(n_band, SNR))
    testY = np.load('../../../npy_data/set_data2/%dband/%ddb_mic_testY.npy'\
                    %(n_band, SNR))
...
#     model.load_state_dict(torch.load('ckpt/model%d_mem/%.1f_ckpt_2000.pt'%(model_num, time_len)))
    ...
    for epoch in range(1001):
        ...
        if epoch%50==0:
            ...
            torch.save(model.state_dict(),'../../../ckpt/model%d/set_data2/%dband/%ddb_ckpt_%d.pt'\
                       %(model_num, n_band, SNR, epoch))
            
            np.savetxt('../../../result/model%d/set_data2/%dband/%ddb_loss_tr.txt'\
                       %(model_num, n_band, SNR), train_loss)
            np.savetxt('../../../result/model%d/set_data2/%dband/%ddb_loss_te.txt'\
                       %(model_num, n_band, SNR), test_loss)
            np.savetxt('../../../result/model%d/set_data2/%dband/%ddb_acc_tr.txt'\
                       %(model_num, n_band, SNR), train_acc)
            np.savetxt('../../../result/model%d/set_data2/%dband/%ddb__acc_te.txt'\
                       %(model_num, n_band, SNR), test_acc)
