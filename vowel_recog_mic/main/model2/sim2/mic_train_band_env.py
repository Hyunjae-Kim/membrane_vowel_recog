...
parser.add_argument('--n_band', type=int)
...
if __name__=='__main__':
    print('[Training]band_env_%dch_%.1f_model%d'%(ch_num, time_len, model_num))
    torch.manual_seed(37)
    torch.cuda.manual_seed_all(37)
    torch.backends.cudnn.deterministic = True
    
    trainX = np.load('../../../npy_data/set_data%d_band_env/%dch_%.1fms_trainX.npy'\
                     %(data_num, ch_num, time_len))
    trainY = np.load('../../../npy_data/set_data%d_band_env/%dch_%.1fms_trainY.npy'\
                     %(data_num, ch_num, time_len))
    testX = np.load('../../../npy_data/set_data%d_band_env/%dch_%.1fms_testX.npy'\
                    %(data_num, ch_num, time_len))
    testY = np.load('../../../npy_data/set_data%d_band_env/%dch_%.1fms_testY.npy'\
                    %(data_num, ch_num, time_len))
    
    trainX2 = np.load('../../../npy_data/set_data%d_band_env/%dch_%.1fms_tr_envX.npy'\
                     %(data_num, ch_num, time_len))
    testX2 = np.load('../../../npy_data/set_data%d_band_env/%dch_%.1fms_te_envX.npy'\
                    %(data_num, ch_num, time_len))
    
    trainX[:, :n_band] = trainX2[:, :n_band].copy()
    testX[:, :n_band] = testX2[:, :n_band].copy()
    ...
    print('%dch_band_env data shape  -  %.1f ms'%(ch_num, time_len))
...
#     model.load_state_dict(torch.load('ckpt/model%d_mem/%.1f_ckpt_2000.pt'%(model_num, time_len)))
    ...
    for epoch in range(1001):
        train_loss, train_acc = train(model, trainX, trainY, batch_tr, device, optimizer, train_loss, train_acc)
        test_loss, test_acc = test(model, testX, testY, batch_te, device, test_loss, test_acc)
        scheduler.step()
        ...
        if epoch%50==0:
            ...
            torch.save(model.state_dict(),'../../../ckpt/model%d/set_data%d_band_env/%d_%dch_%.1f_ckpt_%d.pt'\
                       %(model_num, data_num, n_band, ch_num, time_len, epoch))
            
            np.savetxt('../../../result/model%d/set_data%d_band_env/%d_%dch_%.1f_loss_tr.txt'\
                       %(model_num, data_num, n_band, ch_num, time_len), train_loss)
            np.savetxt('../../../result/model%d/set_data%d_band_env/%d_%dch_%.1f_loss_te.txt'\
                       %(model_num, data_num, n_band, ch_num, time_len), test_loss)
            np.savetxt('../../../result/model%d/set_data%d_band_env/%d_%dch_%.1f_acc_tr.txt'\
                       %(model_num, data_num, n_band, ch_num, time_len), train_acc)
            np.savetxt('../../../result/model%d/set_data%d_band_env/%d_%dch_%.1f_acc_te.txt'\
                       %(model_num, data_num, n_band, ch_num, time_len), test_acc)
