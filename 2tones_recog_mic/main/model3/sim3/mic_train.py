...
if __name__=='__main__':
    ...
    trainX = np.load('../../../npy_data/sim_data%d/%dch/%dch_%.1fms_trainX.npy'\
                     %(data_num, ch_num, ch_num, time_len)).reshape(-1, in_dim)
    trainY = np.load('../../../npy_data/sim_data%d/%dch/%dch_%.1fms_trainY.npy'\
...
#                                      %(model_num, data_num, ch_num, time_len)))
    ...
    for epoch in range(1001):
        ...
        if epoch%100==0:
            ...
            torch.save(model.state_dict(),'../../../ckpt/model%d/sim_data%d/%dch/%.1f_ckpt_%d.pt'\
                       %(model_num, data_num, ch_num, time_len, epoch))
            
            np.savetxt('../../../result/model%d/sim_data%d/%dch/%.1f_loss_tr.txt'\
                       %(model_num, data_num, ch_num, time_len), train_loss)
            np.savetxt('../../../result/model%d/sim_data%d/%dch/%.1f_loss_te.txt'\
                       %(model_num, data_num, ch_num, time_len), test_loss)
            np.savetxt('../../../result/model%d/sim_data%d/%dch/%.1f_acc_tr.txt'\
                       %(model_num, data_num, ch_num, time_len), train_acc)
            np.savetxt('../../../result/model%d/sim_data%d/%dch/%.1f_acc_te.txt'\
