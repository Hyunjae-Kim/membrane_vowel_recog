...
if __name__=='__main__':
    print('[Training]raw_fft_%.1f_model%d'%(time_len, model_num))
    torch.manual_seed(37)
    torch.cuda.manual_seed_all(37)
    torch.backends.cudnn.deterministic = True
    
    trainX = np.load('../../../npy_data/sim_data%d/raw_fft/raw_fft_%.1fms_trainX.npy'\
                     %(data_num, time_len)).reshape(-1,1,point_len)
    trainY = np.load('../../../npy_data/sim_data%d/raw_fft/raw_fft_%.1fms_trainY.npy'\
                     %(data_num, time_len))
    testX = np.load('../../../npy_data/sim_data%d/raw_fft/raw_fft_%.1fms_testX.npy'\
                    %(data_num, time_len)).reshape(-1,1,point_len)
    testY = np.load('../../../npy_data/sim_data%d/raw_fft/raw_fft_%.1fms_testY.npy'\
                    %(data_num, time_len))
    ...
    print('raw_fft data shape  -  %.1f ms'%(time_len))
...
#     model.load_state_dict(torch.load('ckpt/model%d_mem/%.1f_ckpt_2000.pt'%(model_num, time_len)))
    ...
    for epoch in range(1001):
        ...
        if epoch%100==0:
            ...
            torch.save(model.state_dict(),'../../../ckpt/model%d/sim_data%d/raw_fft/%.1f_ckpt_%d.pt'\
                       %(model_num, data_num, time_len, epoch))
            
            np.savetxt('../../../result/model%d/sim_data%d/raw_fft/%.1f_loss_tr.txt'\
                       %(model_num, data_num, time_len), train_loss)
            np.savetxt('../../../result/model%d/sim_data%d/raw_fft/%.1f_loss_te.txt'\
                       %(model_num, data_num, time_len), test_loss)
            np.savetxt('../../../result/model%d/sim_data%d/raw_fft/%.1f_acc_tr.txt'\
                       %(model_num, data_num, time_len), train_acc)
            np.savetxt('../../../result/model%d/sim_data%d/raw_fft/%.1f_acc_te.txt'\
