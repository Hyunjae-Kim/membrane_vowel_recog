...
for freq1 in range(600, 700, 5):
    for freq2 in range(freq1+5, 700, 5):
        ...
        for try_num in range(100):
            sound1 = np.sin(2*np.pi*(freq1*np.linspace(0,time_len/1000,itmax)+np.random.random()))
            sound2 = np.sin(2*np.pi*(freq2*np.linspace(0,time_len/1000,itmax)+np.random.random()))
            sound = sound1 + sound2
