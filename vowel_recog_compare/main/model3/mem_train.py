...
class Model(nn.Module):
    def __init__(self, loss):
        ...
        self.fc1 = nn.Sequential(
            nn.Linear(point_len*10, nodes*4),
...
if __name__=='__main__':
    ...
    trainX = np.load('../../npy_data/%s/%s_%.1fms_trainX.npy'%(data_type, data_type, time_len)).reshape(-1,point_len*10)
    trainY = np.load('../../npy_data/%s/%s_%.1fms_trainY.npy'%(data_type, data_type, time_len))
    testX = np.load('../../npy_data/%s/%s_%.1fms_testX.npy'%(data_type, data_type, time_len)).reshape(-1,point_len*10)
