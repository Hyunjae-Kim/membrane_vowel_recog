...
if __name__=='__main__':
    ...
    trainX = np.load('../../npy_data/mem/mem_%.1fms_trainX.npy'%(time_len))
    trainY = np.load('../../npy_data/mem/mem_%.1fms_trainY.npy'%(time_len))
    testX = np.load('../../npy_data/mem/mem_%.1fms_testX.npy'%(time_len))
    testY = np.load('../../npy_data/mem/mem_%.1fms_testY.npy'%(time_len))
    
    trainX2 = np.load('../../npy_data/mem_env/mem_env_%.1fms_trainX.npy'%(time_len))
    testX2 = np.load('../../npy_data/mem_env/mem_env_%.1fms_testX.npy'%(time_len))
    ...
    print('mem_band_env data shape  -  %.1f ms'%time_len)
...
#     model.load_state_dict(torch.load('ckpt/model%d_mem/%.1f_ckpt_2000.pt'%(model_num, time_len)))
    ...
    for epoch in range(1001):
        ...
        if epoch%50==0:
            ...
            torch.save(model.state_dict(),'../../ckpt/model%d/%s/%d_%.1f_ckpt_%d.pt'%(model_num, data_type, n_band, time_len, epoch))
            np.savetxt('../../result/model%d/%s/%d_%.1f_loss_tr.txt'%(model_num, data_type, n_band, time_len), train_loss)
            np.savetxt('../../result/model%d/%s/%d_%.1f_loss_te.txt'%(model_num, data_type, n_band, time_len), test_loss)
            np.savetxt('../../result/model%d/%s/%d_%.1f_acc_tr.txt'%(model_num, data_type, n_band, time_len), train_acc)
            np.savetxt('../../result/model%d/%s/%d_%.1f_acc_te.txt'%(model_num, data_type, n_band, time_len), test_acc)
