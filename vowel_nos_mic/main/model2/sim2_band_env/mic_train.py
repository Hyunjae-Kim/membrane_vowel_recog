...
if __name__=='__main__':
    print('[Training]%d_%ddb_%.1f_model%d'%(n_band, SNR, time_len, model_num))
    torch.manual_seed(37)
    torch.cuda.manual_seed_all(37)
    torch.backends.cudnn.deterministic = True
    
    trainX = np.load('../../../npy_data/set_data%d_band_env/%ddb/%ddb_mic_%.1fms_trainX.npy'\
                     %(data_num, SNR, SNR, time_len))
    trainY = np.load('../../../npy_data/set_data%d_band_env/%ddb/%ddb_mic_%.1fms_trainY.npy'\
                     %(data_num, SNR, SNR, time_len))
    testX = np.load('../../../npy_data/set_data%d_band_env/%ddb/%ddb_mic_%.1fms_testX.npy'\
                    %(data_num, SNR, SNR, time_len))
    testY = np.load('../../../npy_data/set_data%d_band_env/%ddb/%ddb_mic_%.1fms_testY.npy'\
                    %(data_num, SNR, SNR, time_len))
    
    trainX2 = np.load('../../../npy_data/set_data%d_band_env/%ddb/%ddb_mic_%.1fms_tr_envX.npy'\
                     %(data_num, SNR, SNR, time_len))
    testX2 = np.load('../../../npy_data/set_data%d_band_env/%ddb/%ddb_mic_%.1fms_te_envX.npy'\
                    %(data_num, SNR, SNR, time_len))
    ...
    print('%d band, %ddb data shape  -  %.1f ms'%(n_band, SNR, time_len))
...
#     model.load_state_dict(torch.load('ckpt/model%d_mem/%.1f_ckpt_2000.pt'%(model_num, time_len)))
    ...
    for epoch in range(1001):
        ...
        if epoch%50==0:
            ...
            torch.save(model.state_dict(),'../../../ckpt/model%d/set_data%d_band_env/%dband/%ddb_%.1f_ckpt_%d.pt'\
                       %(model_num, data_num, n_band, SNR, time_len, epoch))
            
            np.savetxt('../../../result/model%d/set_data%d_band_env/%dband/%ddb_%.1f_loss_tr.txt'\
                       %(model_num, data_num, n_band, SNR, time_len), train_loss)
            np.savetxt('../../../result/model%d/set_data%d_band_env/%dband/%ddb_%.1f_loss_te.txt'\
                       %(model_num, data_num, n_band, SNR, time_len), test_loss)
            np.savetxt('../../../result/model%d/set_data%d_band_env/%dband/%ddb_%.1f_acc_tr.txt'\
                       %(model_num, data_num, n_band, SNR, time_len), train_acc)
            np.savetxt('../../../result/model%d/set_data%d_band_env/%dband/%ddb_%.1f_acc_te.txt'\
                       %(model_num, data_num, n_band, SNR, time_len), test_acc)
